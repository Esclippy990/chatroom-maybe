...
<small>IP addresses are considered sensitive information, as it is NEVER recommended to share someone's ip without their permission, not even to the person himself.</small>
  ...
  fetch('https://chatroom-lhb8nu7p.b4a.run//submit', {
    method: 'POST',
    headers: {
...
`
  ...
  let muted = false;
  function recon() {
  ws = new WebSocket('https://chatroom-lhb8nu7p.b4a.run//');
  ws.onopen = () => {
    connected = true;
  ...
  /*let m = new File([msg.data], String(Math.random()).substring(2)+thing.split('/')[1], {
    ...
    let k = 'https://chatroom-lhb8nu7p.b4a.run/file:'+fileID
    let img;
...
//console.log(URL.createObjectURL(m))
  ...
  document.getElementById('send').disabled = true;
  document.getElementById('message').disabled = true;
  fetch('https://chatroom-lhb8nu7p.b4a.run/')
  setTimeout(() => {
  recon()
